...
if (!isset($_POST["token"]) || !isset($_SESSION["token"]) || !isset($_SESSION["token-expire"])) {
    ...
    if($check ==1){
        ...
        $permAdd['empno'] = $id;
        $permAdd['sameAddress'] = sanitize(strtoupper($check));
        $permAdd['permNumAdd'] = sanitize(strtoupper($_POST['AddHouseNumber']));
        $permAdd['permStreet'] = sanitize(strtoupper($_POST['AddStreet']));
        $permAdd['permSubd'] = sanitize(strtoupper($_POST['AddSubd']));
        $permAdd['permZipCode'] = sanitize(strtoupper($_POST['AddZipCode']));
        $permAdd['permRegion'] = sanitize(strtoupper($_POST['AddRegion']));
        $permAdd['permProvince'] = sanitize(strtoupper($_POST['AddProvince']));
        $permAdd['permCity'] = sanitize(strtoupper($_POST['AddCity']));
        $permAdd['permBarangay'] = sanitize(strtoupper($_POST['AddBarangay']));
