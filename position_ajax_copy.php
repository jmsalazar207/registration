...
// 4. Handle search conditions
if (!empty($searchValue)) {
    $userProfileIdsCondition = !empty($limitedUserProfileIds) ? 
                                        "OR pos.position_id IN (" . implode(',', $limitedUserProfileIds) . ")" : "";
    ...
    $mainQuery .= " AND (
        ...
        $userProfileIdsCondition
    )";
}

// 5. Get filtered count
$totalRecordwithFilter = $dbConn->findFirstQuery("SELECT COUNT(pos.position_id) as allcount " . $mainQuery)['allcount'];
