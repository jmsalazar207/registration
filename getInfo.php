...
require_once("includes/dencryption.php");
    $data = [];
    $user = []; 
    $empNumber = $_SESSION['userID'];
    $sql = "SELECT up.empno,up.date_filled,up.fname,up.mname, up.sname, up.ename, up.mobile, 
    up.telephone, up.sex, up.birthdate, up.eaddress, up.numAdd, up.street, up.subd, r.region_name, prov.prov_name,las.area_assignment_name,
    ...
    oi.q6_details,oi.q7a,oi.q7a_details,oi.q7b,oi.q7b_details,oi.q7c,oi.q7c_details,gi.id,gi.govern_id_title,gi.govern_id_no,gi.govern_id_date,gi.govern_id_place,bd.bank_account_number,bank_account_number_hash,bd.bank_account_remarks,bd.bank_account_status
    FROM userprofile up
    ...
    LEFT JOIN lib_bank_details bd ON bd.empno = up.empno
    ...
    if($data) {
        ...
        $data['bankNumber'] = decryptData($data['bank_account_number_hash']);
    }else{
